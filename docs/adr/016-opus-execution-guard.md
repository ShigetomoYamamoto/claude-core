...
> **注記（2026-07-04）**: ガード対象は [ADR-020](./020-thinking-tier-execution-guard.md) により「Opus のみ」→「思考ティア（Opus / Fable / Mythos）」へ拡張された。本文の「Opus」は決定当時の対象。

> **注記（2026-08-09）**: 判定軸そのものが [ADR-026](./026-execution-guard-role-axis.md) により「モデル（思考ティアか）」→「役割（`agent_id` の有無＝メインループか実行層か）」へ置き換えられた。本文が述べるモデル判定と transcript 読み取りは、現在の実装には存在しない。
